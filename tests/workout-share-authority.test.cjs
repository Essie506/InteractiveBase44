...
function resolveWorkoutShareAuthority(workout, user, checkPermissionFn) {
  ...
  if (workout.owner_type === 'business' && workout.business_id) {
    try {
      return checkPermissionFn(workout.business_id, user.id, 'manage_workouts');
    } catch {
      return false;
    }
  }
...
describe('Workout Share UI authority (Spec 12 §9 + Business §8)', () => {
  it('professional creator may share their own workout', () => {
    const workout = {
      owner_type: 'identity',
  ...
  it('unrelated authenticated user may NOT share another professional\'s workout', () => {
    const workout = {
      owner_type: 'identity',
      creator_identity_id: 'identity-A',
      owner_id: 'identity-A',
  ...
  it('business owner with manage_workouts may share business workout', () => {
    const workout = {
      owner_type: 'business',
      business_id: 'biz-1',
  ...
  it('business staff WITHOUT manage_workouts may NOT share business workout', () => {
    const workout = {
      owner_type: 'business',
      business_id: 'biz-1',
      creator_identity_id: 'identity-staff',
  ...
  it('unauthenticated user (null user) may NOT share', () => {
    const workout = {
      owner_type: 'identity',
      creator_identity_id: 'identity-A',
      owner_id: 'identity-A',
