...
import { listBusinessBookings, cancelBooking, completeBooking, rescheduleBooking } from '@/services/bookingService';
...
const STATUS_LABELS = {
  ...
  no_show_provider: 'No-show (business)',
...
function BookingDetail({ booking, onClose, onUpdate }) {
  ...
  const canComplete = ['scheduled', 'confirmed'].includes(booking.booking_status);
  const canReschedule = ['scheduled', 'confirmed'].includes(booking.booking_status) && !booking.event_id;

  const handleCancel = async () => {
    setActionLoading(true);
    try {
      await cancelBooking(booking.id, 'Cancelled by business');
  ...
  const handleReschedule = async () => {
    ...
    try {
      ...
      await rescheduleBooking(booking.id, start.toISOString(), end.toISOString(), 'Rescheduled by business');
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-5 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-stone-400">{booking.provider_initiated ? 'Business-created' : 'Customer-created'}</span>
          </div>
...
export default function BusinessBookings() {
  ...
  const loadBookings = async () => {
    if (!id) return;
    try {
      const bks = await listBusinessBookings(id);
      setBookings(bks);
    } catch {
      setBookings([]);
    } finally {
