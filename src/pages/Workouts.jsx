...
import { listPublishedWorkouts, listMyWorkouts, listSavedWorkouts, listBusinessMyWorkouts } from '@/services/workoutService';
...
export default function Workouts() {
  const { user } = useAuth();
  const { canCreate } = useWorkoutCreateAuthority();
      ...
      try {
        ...
        if (tab === 'mine' && user) {
          if (user.active_context === 'business' && user.active_business_id) {
            list = await listBusinessMyWorkouts(user.active_business_id);
    ...
    <div className="p-6 md:p-10 max-w-6xl mx-auto">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((w) => <WorkoutCard key={w.id} workout={w} showCreator={tab === 'mine' && user?.active_context === 'business'} />)}
        </div>
      )}
    </div>
  );
