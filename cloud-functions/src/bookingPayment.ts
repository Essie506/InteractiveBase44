...
export const createBookingDraft = onCall(
  ...
  async (request) => {
    ...
    if (!callerIdentityId && (!guest || !guest.email)) {
      throw new HttpsError('invalid-argument', 'Guest email required for guest checkout');
    }
    ...
    if (needsStripe) {
      const ready = await isPaymentReady(db, provider_identity_id, business_id || null);
      if (!ready) {
    ...
    // ── One-to-one booking: overlap conflict detection + availability ──
    // Calendar is authoritative for availability (§27, §32): Booking
    // requests availability from Calendar before holding a slot.
    ...
    const holdResult = await db.runTransaction(async (tx) => {
      ...
      // Range overlap (not exact-time equality) so partially overlapping
      // commitments are detected. Revalidated inside the transaction.
      if (await hasOverlappingHold(tx, provider_identity_id, start_time, end_time)) {
        throw new HttpsError('failed-precondition', 'This slot overlaps an active hold');
      }
      if (await hasOverlappingBooking(tx, provider_identity_id, start_time, end_time)) {
...
export const createPaymentIntent = onCall(
  ...
  async (request) => {
    ...
    if (booking.payment_requirement !== 'required') {
      throw new HttpsError('failed-precondition', 'This booking does not require payment');
    }
    ...
    if (!holdDoc.exists || holdDoc.data()!.status !== 'active') {
      throw new HttpsError('failed-precondition', 'Slot hold expired. Please restart booking.');
    }
    ...
    const connectedAccount = await resolveConnectedAccount(
      db, booking.provider_identity_id, booking.business_id,
    );
    if (!connectedAccount || !connectedAccount.chargesEnabled) {
      throw new HttpsError('failed-precondition', 'Provider is not payment-ready');
    }
