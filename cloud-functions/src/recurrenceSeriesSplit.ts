...
const SOURCE_SPLIT_AUTHORITY: Record<string, boolean> = {
  ...
  async (request) => {
    ...
    if (!isSourceAuthorised) {
      throw new HttpsError(
        'failed-precondition',
        `Source system '${sourceSystem}' owns this series. ` +
    ...
    for (const exc of exceptions) {
      if (new Date(exc.original_start_time).getTime() >= splitDate.getTime()) {
        ...
        await db.collection(EXCEPTIONS).doc(exceptionDocId(series_event_id, exc.original_start_time)).delete().catch(() => {});
      }
    }
