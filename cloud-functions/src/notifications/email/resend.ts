...
import { EmailProvider, EmailSendRequest, EmailSendResult, EmailError } from './types';
...
export class ResendEmailProvider implements EmailProvider {
  ...
  async send(req: EmailSendRequest): Promise<EmailSendResult> {
    if (!this.apiKey) {
      throw new EmailError('RESEND_API_KEY not configured', 'config_missing', false);
    }
    let res: Response;
    try {
      ...
      throw new EmailError(`Network error: ${err?.message || err}`, 'network', true);
    }

    if (res.status === 429 || res.status >= 500) {
      const errText = await res.text().catch(() => '');
      throw new EmailError(`Resend transient ${res.status}: ${errText.slice(0, 200)}`, 'transient', true);
    }
    if (res.status === 422) {
      const errText = await res.text().catch(() => '');
      throw new EmailError(`Resend invalid request: ${errText.slice(0, 200)}`, 'invalid', false);
    }
    if (!res.ok) {
      const errText = await res.text().catch(() => '');
      throw new EmailError(`Resend error ${res.status}: ${errText.slice(0, 200)}`, 'error', false);
    }
