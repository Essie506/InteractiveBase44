...
export interface CalendarEmailContext {
  ...
  timeLabel: string;       // e.g. "10:30 – 11:30" or "All day"
  timezone: string;        // IANA tz
  safeLocationLabel: string | null;
  eventLink: string;       // /calendar?event=...
...
const TEMPLATES: Record<
  CalendarEventType,
  { subject: (t: CalendarEmailContext) => string; intro: (t: CalendarEmailContext) => string }
> = {
  calendar_event_invited: {
...
export function buildCalendarEmailPayload(ctx: CalendarEmailContext): BuiltEmail {
