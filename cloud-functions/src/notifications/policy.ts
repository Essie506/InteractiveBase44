...
export type PolicyValue = 'required' | 'conditional' | 'prohibited';

export const DELIVERY_POLICY: Record<string, Record<Channel, PolicyValue>> = {
  // Verification
...
export function resolveDeliveryPolicy(eventType: string, channel: Channel): PolicyValue {
...
export function resolveChannels(
  ...
  channelsToConsider: Channel[] = ['in_app', 'email'],
): Channel[] {
  const out: Channel[] = [];
  for (const ch of channelsToConsider) {
    ...
    if (policy === 'required') {
      ...
      if (!prefs) {
        ...
        if (prefs[prefKey] !== false) out.push(ch);
      }
    }
    // prohibited → skip
  }
