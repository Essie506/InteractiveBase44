...
export const CAPACITY_CONSUMING_STATES: string[] = [
  'requested',
...
export const CAPACITY_RELEASING_STATES: string[] = [
  'draft',
...
export function isCapacityConsuming(status: string | null | undefined): boolean {
  return !!status && CAPACITY_CONSUMING_STATES.includes(status);
}
 ...
 */
export function normaliseAttendeeQuantity(qty: any): number {
 ...
 */
export function sumAttendeeQuantity(bookingDocs: Array<{ data: () => any }>): number {
  let total = 0;
 ...
 */
export function resolveEventPrice(eventData: any): {
  price_pence: number;
  currency: string;
  is_free: boolean;
} {
  ...
  if (typeof pricePence === 'number' && Number.isFinite(pricePence)) {
    if (pricePence === 0) return { price_pence: 0, currency, is_free: true };
    return { price_pence: Math.floor(pricePence), currency, is_free: false };
  }
