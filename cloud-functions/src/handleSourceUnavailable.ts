 ...
 */
export function resolveUnavailableTransition(
  currentLifecycleState: string,
  reason: string,
): { newLifecycleState: string | null; redactDetail: boolean; removeFromPublic: boolean } {
...
export const handleSourceUnavailable = onCall(
  ...
  async (request) => {
    ...
    if (!source_system || !source_id) {
      throw new HttpsError('invalid-argument', 'source_system and source_id are required');
    }
    if (!VALID_REASONS.has(reason)) {
      throw new HttpsError('invalid-argument', `Invalid reason: ${reason}. Must be one of: ${Array.from(VALID_REASONS).join(', ')}`);
    }
    ...
    const transitions: Array<{ event_id: string; new_state: string | null }> = [];

    for (const doc of snap.docs) {
      const eventData = doc.data()!;
      const currentLifecycle = eventData.lifecycle_state || 'scheduled';
      const { newLifecycleState, redactDetail, removeFromPublic } =
        resolveUnavailableTransition(currentLifecycle, reason);
