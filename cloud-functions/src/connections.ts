...
import { db, allowedOrigins, getIdentityId, isBlocked, hasAcceptedConnection, connectionPairId } from './shared';
...
export const createConnectionRequest = onCall(
  { region: 'europe-west2', cors: allowedOrigins },
  async (request) => {
    if (!request.auth) {
      throw new HttpsError('unauthenticated', 'Authentication required');
    }
    const requesterId = await getIdentityId(request.auth.uid);
    ...
    const blocked = await isBlocked(requesterId, target_id);
    if (blocked) {
      throw new HttpsError('permission-denied', 'Cannot request connection — blocking relationship exists');
    }

    const pairId = connectionPairId(requesterId, target_id);

    // Already connected? Return the existing active connection.
    const existingConn = await db.collection(CONNECTIONS).doc(pairId).get();
    ...
    const existingPending = await db.collection(REQUESTS)
...
export const respondConnectionRequest = onCall(
  { region: 'europe-west2', cors: allowedOrigins },
  async (request) => {
    if (!request.auth) {
      throw new HttpsError('unauthenticated', 'Authentication required');
    ...
    if (!request_id || !['accept', 'decline'].includes(action)) {
      throw new HttpsError('invalid-argument', 'request_id and response (accept|decline) required');
    }

    const reqRef = db.collection(REQUESTS).doc(request_id);
    ...
    if (reqData.status !== 'pending') {
      throw new HttpsError('failed-precondition', 'Connection request is not pending');
    }
    // Only the target can respond — requester cannot self-accept.
    if (reqData.target_id !== callerId) {
      throw new HttpsError('permission-denied', 'Only the recipient can respond to a connection request');
    }

    const now = new Date().toISOString();
    const pairId = connectionPairId(reqData.requester_id, reqData.target_id);
    ...
    if (action === 'accept') {
      ...
      await db.runTransaction(async (tx) => {
        ...
        if (snap.data()!.status !== 'pending') {
          throw new HttpsError('failed-precondition', 'Connection request is no longer pending');
        }
        tx.update(reqRef, { status: 'accepted', responded_at: now, _updated_date: now });

        const connRef = db.collection(CONNECTIONS).doc(pairId);
...
export const disconnectConnection = onCall(
  { region: 'europe-west2', cors: allowedOrigins },
  async (request) => {
    if (!request.auth) {
      throw new HttpsError('unauthenticated', 'Authentication required');
    ...
    const pairId = connectionPairId(callerId, target_id);
    const connRef = db.collection(CONNECTIONS).doc(pairId);
    ...
    if (conn.identity_a_id !== callerId && conn.identity_b_id !== callerId) {
      throw new HttpsError('permission-denied', 'Only a participant can disconnect a connection');
    }
...
export const resolveProfessionalAccess = onCall(
  ...
  async (request) => {
    ...
    if (request.auth) {
      try {
        callerId = await getIdentityId(request.auth.uid);
      } catch {
        callerId = null;
      }
    }
    ...
    const pubDoc = await db.collection(PUBLIC).doc(screenName).get();
    if (pubDoc.exists) {
    ...
    const privateSnap = await db.collection(PROFILES)
    ...
    if (callerId && ownerId === callerId) {
      return {
        access: 'owner',
        profile: buildPublicProjection(ownerId, profileId, profileData, null),
        is_owner: true,
      };
    }
    ...
    if (visibility === 'connections') {
      if (!callerId) {
        ...
        if (directoryVisibility === 'listed') {
          return { access: 'restricted', profile: buildDirectoryEntry(ownerId, profileId, profileData, null), is_owner: false };
        }
        return { access: 'denied', profile: null, is_owner: false };
      }
      const connected = await hasAcceptedConnection(callerId, ownerId);
      if (connected) {
        return {
          access: 'connection',
          profile: buildPublicProjection(ownerId, profileId, profileData, null),
...
// Request: { target_id }
// Returns: { status }
export const resolveConnectionStatus = onCall(
  { region: 'europe-west2', cors: allowedOrigins },
  async (request) => {
    if (!request.auth) {
      throw new HttpsError('unauthenticated', 'Authentication required');
...
export const resolveConnectionStatuses = onCall(
  { region: 'europe-west2', cors: allowedOrigins },
  async (request) => {
    if (!request.auth) {
      throw new HttpsError('unauthenticated', 'Authentication required');
    ...
    const callerId = await getIdentityId(request.auth.uid);
    const targetIds: string[] = Array.isArray(request.data?.target_ids) ? request.data.target_ids : [];
...
async function computeStatusMap(callerId: string, targetIds: string[]): Promise<Record<string, string>> {
  const [outgoingSnap, incomingSnap, connASnap, connBSnap, blockOutSnap, blockInSnap] = await Promise.all([
    db.collection(REQUESTS).where('requester_id', '==', callerId).get(),
    db.collection(REQUESTS).where('target_id', '==', callerId).get(),
    db.collection(CONNECTIONS).where('identity_a_id', '==', callerId).get(),
    db.collection(CONNECTIONS).where('identity_b_id', '==', callerId).get(),
    db.collection('blockRecords').where('blocker_id', '==', callerId).get(),
    db.collection('blockRecords').where('blocked_id', '==', callerId).get(),
  ]);
  ...
  const incomingPending = new Set(
    incomingSnap.docs
      .map((d) => d.data())
      .filter((d) => d.status === 'pending')
      .map((d) => d.requester_id as string)
  );
  const connStatus = new Map<string, string>();
  for (const doc of [...connASnap.docs, ...connBSnap.docs]) {
    ...
    connStatus.set(other as string, data.status as string);
  }
  const blockedByCaller = new Set(blockOutSnap.docs.map((d) => d.data().blocked_id as string));
  const blockedCaller = new Set(blockInSnap.docs.map((d) => d.data().blocker_id as string));
