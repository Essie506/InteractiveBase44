...
exports.seriesIdFromOccurrence = seriesIdFromOccurrence;
...
function expandOccurrences(seriesEventId, rruleStr, dtStartIso, dtEndIso, rangeStartIso, rangeEndIso, maxOccurrences = 365) {
    ...
    if (rule.freq === 'DAILY') {
        ...
        while (occurrences.length < maxOccurrences) {
            ...
            if (new Date(year, month, 1) > effectiveUntil)
                break;
        }
    }
        ...
        while (occurrences.length < maxOccurrences) {
            ...
            if (!tryOccurrence(occ))
                break;
            year += rule.interval;
            if (new Date(year, 0, 1) > effectiveUntil)
                break;
        }
    }
    return occurrences;
...
function seriesIdFromOccurrence(occId) {
