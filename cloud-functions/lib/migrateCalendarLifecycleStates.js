...
exports.migrateCalendarLifecycleStates = void 0;
exports.migrateLifecycleState = migrateLifecycleState;
...
function migrateLifecycleState(event) {
    ...
    if (UNAMBIGUOUS_V2_STATES.has(current))
        return null;
...
exports.migrateCalendarLifecycleStates = (0, https_1.onCall)({ region: 'europe-west2', cors: shared_1.allowedOrigins }, async (request) => {
    if (!request.auth) {
    ...
    const batchSize = 500;
    while (true) {
        let q = shared_1.db.collection(EVENTS)
...
//# sourceMappingURL=migrateCalendarLifecycleStates.js.map