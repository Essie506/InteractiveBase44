 ...
 */
function isEventEligible(data, nowMs) {
    if (!data)
        return false;
 ...
 */
function isEventListable(data, hostData, nowMs) {
    if (!isEventEligible(data, nowMs))
        return false;
    if (!hostData)
        return false;
    if (hostData.visibility !== 'public')
        return false;
 ...
 */
function computeAvailability(capacity, reservedCount) {
...
//# sourceMappingURL=eventProjectionEligibility.js.map