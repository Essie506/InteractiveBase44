...
exports.setPersonalTimelineState = exports.revokeCalendarInvitation = exports.respondCalendarInvitation = void 0;
exports.participationDocId = participationDocId;
exports.syncParticipationRecords = syncParticipationRecords;
exports.revokeParticipationRecords = revokeParticipationRecords;
...
function participationDocId(eventId, identityId) {
...
async function syncParticipationRecords(eventId, invitedIdentityIds, invitedAtIso) {
    for (const identityId of invitedIdentityIds) {
        if (!identityId)
            continue;
        const docId = participationDocId(eventId, identityId);
        const ref = shared_1.db.collection(PARTICIPATION).doc(docId);
...
async function revokeParticipationRecords(eventId, removedIdentityIds, revokedBy, revokedAtIso) {
    for (const identityId of removedIdentityIds) {
        if (!identityId)
            continue;
        const docId = participationDocId(eventId, identityId);
        await shared_1.db.collection(PARTICIPATION).doc(docId).set({
...
exports.respondCalendarInvitation = (0, https_1.onCall)({ region: 'europe-west2', cors: shared_1.allowedOrigins }, async (request) => {
    if (!request.auth) {
    ...
    const eventSnap = await shared_1.db.collection(EVENTS).doc(event_id).get();
    if (!eventSnap.exists) {
    ...
    if (!invitedIds.includes(callerIdentityId)) {
        throw new https_1.HttpsError('permission-denied', 'You are not invited to this event');
    }
    ...
    if (partData.response_state === 'revoked') {
        throw new https_1.HttpsError('failed-precondition', 'This invitation has been revoked');
    }
    ...
    if (organiserId && organiserId !== callerIdentityId) {
        const eventType = response === 'accepted'
    ...
    await (0, calendarSignal_1.emitCalendarSignal)([callerIdentityId, organiserId]);
...
exports.revokeCalendarInvitation = (0, https_1.onCall)({ region: 'europe-west2', cors: shared_1.allowedOrigins }, async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError('unauthenticated', 'Authentication required');
    }
    const callerIdentityId = await (0, shared_1.getIdentityId)(request.auth.uid);
    ...
    if (!event_id || !identity_id) {
        throw new https_1.HttpsError('invalid-argument', 'event_id and identity_id are required');
    }
    // ── Load the event ──
    const eventSnap = await shared_1.db.collection(EVENTS).doc(event_id).get();
    if (!eventSnap.exists) {
    ...
    if (!isCreator && !isIdentityOwner && !isBizManager) {
        throw new https_1.HttpsError('permission-denied', 'Not authorised to revoke invitations for this event');
    }
    ...
    const updatedInvited = currentInvited.filter((id) => id !== identity_id);
    const nowIso = new Date().toISOString();
    await shared_1.db.collection(EVENTS).doc(event_id).set({
    ...
    await (0, calendarSignal_1.emitCalendarSignal)([identity_id, callerIdentityId]);
    return { revoked: true };
});
...
exports.setPersonalTimelineState = (0, https_1.onCall)({ region: 'europe-west2', cors: shared_1.allowedOrigins }, async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError('unauthenticated', 'Authentication required');
    }
    const callerIdentityId = await (0, shared_1.getIdentityId)(request.auth.uid);
    ...
    if (!validStates.includes(personal_lifecycle_state)) {
        throw new https_1.HttpsError('invalid-argument', "personal_lifecycle_state must be 'completed', 'skipped', 'archived', or null");
    }
    if (typeof hidden_from_timeline !== 'boolean') {
        throw new https_1.HttpsError('invalid-argument', 'hidden_from_timeline must be a boolean');
    }
    // ── Verify the event exists ──
    const eventSnap = await shared_1.db.collection(EVENTS).doc(event_id).get();
    if (!eventSnap.exists) {
    ...
    await (0, calendarSignal_1.emitCalendarSignal)([callerIdentityId]);
    return {
        personal_lifecycle_state: personal_lifecycle_state,
        hidden_from_timeline: hidden_from_timeline,
    };
});
//# sourceMappingURL=calendarParticipation.js.map