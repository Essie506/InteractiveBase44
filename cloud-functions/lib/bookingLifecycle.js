...
exports.completeBooking = exports.reportNoShow = exports.rescheduleBooking = exports.cancelBooking = void 0;
...
exports.cancelBooking = (0, https_1.onCall)({ region: 'europe-west2', cors: shared_1.allowedOrigins, secrets: ['STRIPE_SECRET_KEY'] }, async (request) => {
    ...
    if (booking.booking_status === 'completed') {
        throw new https_1.HttpsError('failed-precondition', 'Cannot cancel a completed booking');
    }
    const now = new Date();
    ...
    if (booking.payment_status_mirror === 'succeeded' && booking.payment_record_id) {
        const payDoc = await shared_1.db.collection('paymentRecords').doc(booking.payment_record_id).get();
        if (payDoc.exists) {
            ...
            if (refundAmountPence > 0) {
                ...
                // Create RefundRecord
                const refundRef = shared_1.db.collection('refundRecords').doc();
...
exports.rescheduleBooking = (0, https_1.onCall)({ region: 'europe-west2', cors: shared_1.allowedOrigins, secrets: ['STRIPE_SECRET_KEY'] }, async (request) => {
    ...
    if (booking.booking_status !== 'scheduled' && booking.booking_status !== 'confirmed') {
        throw new https_1.HttpsError('failed-precondition', 'Only scheduled or confirmed bookings can be rescheduled');
    }
    ...
    // Confirm new hold
    await shared_1.db.collection('slotHolds').doc(holdResult).update({
        status: 'confirmed',
        _updated_date: now,
    });
...
exports.reportNoShow = (0, https_1.onCall)({ region: 'europe-west2', cors: shared_1.allowedOrigins }, async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError('unauthenticated', 'Authentication required');
    }
    const callerIdentityId = await (0, shared_1.getIdentityId)(request.auth.uid);
    ...
    if (!isProvider && !isBizAdmin && !isCustomer) {
        throw new https_1.HttpsError('permission-denied', 'Not authorized to report no-show for this booking');
    }
    ...
    if (booking.booking_status !== 'scheduled' && booking.booking_status !== 'confirmed' && booking.booking_status !== 'completed') {
        throw new https_1.HttpsError('failed-precondition', 'No-show can only be reported for scheduled/completed bookings');
    }
    const now = new Date();
    const endTime = new Date(booking.end_time);
    if (endTime > now) {
        throw new https_1.HttpsError('failed-precondition', 'No-show can only be reported after the booking end time');
    }
    // Idempotency: check if already reported
    if (booking.no_show_state?.reported) {
        throw new https_1.HttpsError('failed-precondition', 'No-show already reported');
    }
    const nowIso = now.toISOString();
    await bookingDoc.ref.update({
...
exports.completeBooking = (0, https_1.onCall)({ region: 'europe-west2', cors: shared_1.allowedOrigins }, async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError('unauthenticated', 'Authentication required');
    }
    const callerIdentityId = await (0, shared_1.getIdentityId)(request.auth.uid);
    ...
    if (!isProvider && !isBizAdmin) {
        throw new https_1.HttpsError('permission-denied', 'Only the provider can complete this booking');
    }
    if (booking.booking_status !== 'scheduled' && booking.booking_status !== 'in_progress' && booking.booking_status !== 'confirmed') {
        throw new https_1.HttpsError('failed-precondition', 'Only scheduled, in-progress, or confirmed bookings can be completed');
    }
    const now = new Date().toISOString();
    await bookingDoc.ref.update({
