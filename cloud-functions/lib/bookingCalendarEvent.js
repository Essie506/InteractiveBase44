...
exports.createBookingCalendarEvent = createBookingCalendarEvent;
exports.createHoldCalendarEvent = createHoldCalendarEvent;
exports.releaseHoldCalendarEvent = releaseHoldCalendarEvent;
 ...
 */
async function createBookingCalendarEvent(bookingId, booking, nowIso) {
...
async function createHoldCalendarEvent(holdId, hold, nowIso) {
    ...
    if (created) {
        await (0, calendarSignal_1.emitCalendarSignal)([hold.provider_identity_id]);
    }
    return { calendar_event_id: calendarEventId, created };
}
...
async function releaseHoldCalendarEvent(holdId, nowIso) {
    const snap = await shared_1.db.collection(EVENTS).where('source_id', '==', `hold:${holdId}`).limit(1).get();
    if (snap.empty)
        return;
    const doc = snap.docs[0];
    const ev = doc.data();
    if (ev.lifecycle_state === 'cancelled' || ev.lifecycle_state === 'removed')
        return;
    ...
    await (0, calendarSignal_1.emitCalendarSignal)([ev.created_by_id, ...(ev.assigned_identity_ids || [])]);
}
