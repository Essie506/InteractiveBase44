...
exports.deactivateIdentityCalendar = void 0;
...
exports.deactivateIdentityCalendar = (0, https_1.onCall)({ region: 'europe-west2', cors: shared_1.allowedOrigins }, async (request) => {
    if (!request.auth) {
    ...
    if (!isSelf && !isPlatformAdmin) {
        throw new https_1.HttpsError('permission-denied', 'Not authorised to deactivate this identity\u2019s calendar');
    }
    ...
    for (const doc of snap.docs) {
        ...
        if (['cancelled', 'removed', 'historical', 'archived', 'superseded'].includes(ev.lifecycle_state))
            continue;
        // Only cancel future-active events; past events become historical naturally.
        if (!FUTURE_ACTIVE_STATES.includes(ev.lifecycle_state))
            continue;
        // Booking-owned events are managed by the Booking system.
        if (ev.source_system === 'booking')
            continue;
        ...
        batch.delete(shared_1.db.collection(PUBLIC).doc(doc.id));
        cancelled++;
...
//# sourceMappingURL=identityDeactivation.js.map