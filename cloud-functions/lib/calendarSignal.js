...
exports.collectAffectedIdentities = collectAffectedIdentities;
exports.emitCalendarSignal = emitCalendarSignal;
exports.emitCalendarSignalForEvent = emitCalendarSignalForEvent;
 ...
 */
async function collectAffectedIdentities(event) {
    if (!event)
        return [];
    const ids = new Set();
    ...
    if (event.owner_type === 'business' && event.business_id) {
        try {
            const snap = await shared_1.db.collection('businessMemberships')
                .where('business_id', '==', event.business_id)
                .get();
            for (const doc of snap.docs) {
                const identityId = doc.data()?.identity_id;
                if (identityId)
                    ids.add(identityId);
            }
        }
 ...
 */
async function emitCalendarSignal(identityIds) {
    const unique = Array.from(new Set((identityIds || []).filter(Boolean)));
    if (unique.length === 0)
        return;
    const nowIso = new Date().toISOString();
    await Promise.all(unique.map((identityId) => shared_1.db.collection(SIGNALS).doc(identityId).set({
...
async function emitCalendarSignalForEvent(event) {
    const ids = await collectAffectedIdentities(event);
    await emitCalendarSignal(ids);
}
//# sourceMappingURL=calendarSignal.js.map