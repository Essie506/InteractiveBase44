...
exports.backfillCalendarOwnership = void 0;
...
async function isBusinessDoc(id) {
    if (!id)
        return false;
    const snap = await shared_1.db.collection(BUSINESSES).doc(id).get();
    return snap.exists;
}
exports.backfillCalendarOwnership = (0, https_1.onCall)({ region: 'europe-west2', cors: shared_1.allowedOrigins }, async (request) => {
    if (!request.auth) {
    ...
    let hasMore = true;
    while (hasMore) {
        let q = shared_1.db.collection(EVENTS).orderBy('__name__').limit(250);
        if (lastDoc)
        ...
        for (const doc of snap.docs) {
            ...
            if (data.owner_type === 'business') {
                ...
                if (!looksLikeBusiness) {
                    ...
                    if (!realBusinessId && data.source_system === 'booking' && data.source_id) {
                        const b = await shared_1.db.collection(BOOKINGS).doc(data.source_id).get();
                        if (b.exists)
                            realBusinessId = b.data().business_id || null;
                    }
                    if (realBusinessId && realBusinessId !== data.owner_id) {
                        ...
                        if (!data.created_by_id)
                            update.created_by_id = formerIdentity;
            ...
            await shared_1.db.collection(EVENTS).doc(id).set(update, { merge: true });
            // Re-read merged data and repair the public projection.
            const merged = (await shared_1.db.collection(EVENTS).doc(id).get()).data();
            await (0, calendarEvent_1.maintainProjection)(id, merged);
            stats.projectionsRepaired++;
        }
    }
    return { status: 'complete', stats };
});
//# sourceMappingURL=backfillCalendarOwnership.js.map