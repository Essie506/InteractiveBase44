...
exports.runProfessionalBackfill = runProfessionalBackfill;
...
async function runProfessionalBackfill() {
    const result = {
        total: 0,
        projected: 0,
    ...
    for (const doc of proSnap.docs) {
        ...
        if (data.directory_visibility === undefined) {
            const wasListable = (0, projectionEligibility_1.isProfessionalListable)(data, canonicalScreenName);
        ...
        const isPublicEligible = (0, projectionEligibility_1.isProfessionalListable)(data, canonicalScreenName);
        const isDirectoryEligible = (0, projectionEligibility_1.isProfessionalDirectoryListable)(data, canonicalScreenName);
        ...
        if (isDirectoryEligible && canonicalScreenName) {
            const locationGeo = await (0, geo_1.fetchProfessionalPublicGeo)(shared_1.db, data.service_area_location_id, data.location_id);
            const advert = (0, professionalProfile_1.buildDirectoryEntry)(data.identity_id, doc.id, data, locationGeo);
            await shared_1.db.collection('professionalDirectoryEntries').doc(canonicalScreenName).set(advert);
            result.directoryEntriesProjected++;
            const staleAdSnap = await shared_1.db.collection('professionalDirectoryEntries')
                .where('identity_id', '==', data.identity_id)
                .get();
...
//# sourceMappingURL=professionalBackfill.js.map